...
void push_one_local(struct Table *table, char *name, int number)
{
	struct Symbol *new_symbol, *ptr;
	new_symbol=(struct Symbol *)malloc(sizeof(struct Symbol));

	new_symbol->name = (char *)malloc(sizeof(char)*(strlen(name)+1));
	strcpy( new_symbol->name, name );

	new_symbol->number=number; 
	new_symbol->next=NULL;
	
	if(table->root==NULL){
		table->root=new_symbol;
	}
	else{
		for(ptr=table->root;ptr->next!=NULL;ptr=ptr->next);
		ptr->next=new_symbol;
	}
}

void delete_all_local(struct Table *table)
{
	struct Symbol *ptr, *next;
	next=NULL;
        ptr=table->root;
	if(ptr==NULL){}
	else{
...
struct Symbol *search_for_symbol(struct Table *table, char *id)
{
        struct Symbol *ptr;
	ptr=(struct Symbol *)malloc(sizeof(struct Symbol));
        ptr=table->root;
        for(ptr=(table->root);ptr!=NULL;ptr=ptr->next){
                if(!strcmp(ptr->name,id))
		{
                        return ptr;
                }
        }
...
void print_local_table(struct Table *table)
{
	int i;
	for(i=0;i< 80;i++)printf("-");
	printf("\n");
	struct Symbol *ptr;
	printf("I'm local table:\n");
	for(ptr=table->root;ptr!=NULL;ptr=ptr->next){
                //printf("%-32s\t", ptr->name);
		printf("%s,%d\n",ptr->name,ptr->number);
        }
